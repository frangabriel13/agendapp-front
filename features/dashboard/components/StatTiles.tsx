...
export function StatTiles({ stats, note }: { stats: Stat[]; note?: React.ReactNode }) {
  return (
    <>
      {note && <div className="mb-2">{note}</div>}
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        {stats.map(({ label, value, hint, icon: Icon }) => (
          /* En fila y no en columna: a todo el ancho, un número chico arriba a
             la izquierda de una tarjeta de 450px deja un vacío enorme. */
          <div
            key={label}
            className={cn(
              cardSurface,
              "relative isolate flex overflow-hidden p-4",
